...
public class DbInitializer
{
    ...
    {
        if (context is null)
            throw new ArgumentNullException($"Dbcontext in class {nameof(DbInitializer)} in method {nameof(SeedData)} cannot be null");

        context.Database.Migrate();

        if (context.Auctions.Any())
        {
            Console.WriteLine("Data is already available");
            return;
        }
